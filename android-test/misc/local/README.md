...
./scripts/local_stats2.0.sh

# Только переустановка (если APK уже собран)
./scripts/local_stats2.0.sh --no-build

# Свежий logcat после запуска
./scripts/local_stats2.0.sh --logs

# После билда — git commit + push (только android-test/)
./scripts/local_stats2.0.sh --push "fix: кнопка назад в под-табах"

# Если install завис — перезапустить adb-server
./scripts/local_stats2.0.sh --reboot-adb

# Комбинируй
./scripts/local_stats2.0.sh --no-build --logs --reboot-adb
```
